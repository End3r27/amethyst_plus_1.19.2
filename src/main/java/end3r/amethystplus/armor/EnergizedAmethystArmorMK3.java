...
public class EnergizedAmethystArmorMK3 extends ArmorItem {
    public static final int MAX_ENERGY = 100000;           // Maximum energy storage
    private static final int ENERGY_DRAIN_PER_SECOND = 15; // Energy consumed per second

    public EnergizedAmethystArmorMK3(EquipmentSlot slot, Settings settings) {
        super(new CustomArmorMaterialMK3(), slot, settings); // Uses a custom armor material
    }
    ...
    public void appendTooltip(ItemStack stack, World world, List<Text> tooltip, TooltipContext context) {
        ...
        tooltip.add(Text.literal("When fully charged, gain Creative Flight,").formatted(Formatting.GRAY));
    }
            ...
            if (isWorn) {
                ...
                if (gameTime % 20 == 0) { // Runs once every 20 ticks (1 second)
                    ...
                    if (currentEnergy > ENERGY_DRAIN_PER_SECOND) {
                        ...
                        player.addStatusEffect(new StatusEffectInstance(StatusEffects.JUMP_BOOST, 220, 1, true, false));
                        player.addStatusEffect(new StatusEffectInstance(StatusEffects.HASTE, 220, 1, true, false));
